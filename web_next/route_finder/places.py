...
def coords_for_place(name: str) -> tuple[float, float] | None:
    resolved = _fast_city_match(name)
    if resolved:
        return (resolved.lat, resolved.lon)
    from route_finder.historic_fares import _coords

    return _coords(name)


def search_places(query: str, *, limit: int = 8) -> list[dict[str, float | str]]:
    q = _normalize(query)
    if not q:
        return [
            {"name": city, "lat": _CITY_COORDS[key][0], "lon": _CITY_COORDS[key][1]}
            for city in MAJOR_EUROPEAN_CITIES[:limit]
            if (key := _normalize(city)) in _CITY_COORDS
        ]

    seen: set[str] = set()
    results: list[tuple[int, str, float, float]] = []

    def add(name: str, lat: float, lon: float, rank: int) -> None:
        key = _normalize(name)
        if key in seen:
            return
        seen.add(key)
        results.append((rank, name, lat, lon))

    for city in MAJOR_EUROPEAN_CITIES:
        label = city.strip()
        key = _normalize(label)
        coords = _CITY_COORDS.get(key) or _CITY_COORDS.get(canonical_place(label))
        if not coords:
            continue
        if key.startswith(q) or q in key:
            add(label, coords[0], coords[1], 0 if key.startswith(q) else 1)
        elif all(part in key for part in q.split()):
            add(label, coords[0], coords[1], 2)
    ...
    for key, coords in _CITY_COORDS.items():
        ...
        if q in key or key.startswith(q):
            name = " ".join(part.capitalize() for part in key.split())
            add(name, coords[0], coords[1], 3)

    results.sort(key=lambda item: (item[0], item[1]))
    return [
        {"name": name, "lat": lat, "lon": lon}
        for _, name, lat, lon in results[:limit]
    ]
